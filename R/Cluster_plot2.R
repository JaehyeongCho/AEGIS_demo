# Cluster.plot2('kulldorff','0.05','no','United States')
# method = 'kulldorff'
# parameter = '0.05'
# GIS.Age = 'no'
# country = 'United States'
Cluster.plot2 <- function(method, parameter, GIS.Age, country){
  switch(method,
         "moran" = {
         ...
         "kulldorff" = {
           ...
           map <-  ggmap::ggmap(get_map(location = c(-74,41
                                                     ,-69.5,43.5), source = 'google',zoom = 7,maptype = 'roadmap'))
           ...
           temp_num <- c()
           gadm_list <- c()
           for (i in 1:length(result$most.likely.cluster$location.IDs.included)){
             temp_num <- result$most.likely.cluster$location.IDs.included[i]
             print(gadm_id[temp_num])
             gadm_list[i] <- gadm_id[temp_num]
           }


           df_2 <- GADM.table
           df_2$cluster <- ifelse(
             GADM.table$ID_2 %in% gadm_list, "Primary cluster",
           ...
           plot <- map +
             ...
             labs(title = primary_labs, subtitle =  secondary_labs)+ theme(legend.key.width=unit(0.5, "cm"), legend.key.height = unit(0.5,"cm"))
         }
  )
  plot <- plot
  return(plot)
